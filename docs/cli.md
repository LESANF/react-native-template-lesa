...
- **레포를 public 으로 바꿔 tarball 받기 → 거부했다가 해소(2026-09-10).** `docs/` 가 참조 앱의
  결함 목록과 자체 OTA 서버 구성, 로컬 절대 경로를 담고 있어 한 번 거부했다. 익명화로 그
  조건을 없앴다 — 레포명·인프라 스택·절대 경로를 걷어냈고 기술 내용만 남겼다.
