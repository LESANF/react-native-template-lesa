/**
 * 경계 규칙이 실제로 발동하는지 eslint 를 돌려서 확인한다. flat config 는 같은 rule 키를
 * 쓰는 블록 중 마지막 것만 적용하므로, 넓은 블록의 규칙이 좁은 블록에 조용히 덮여 사라진
 * 적이 두 번 있다 — providers 역류 error 가 features 에서 비어 있었다.
 *
 * 가상 파일 경로와 stdin으로 검사해 Expo Router·Metro가 테스트 파일을 감지하지 않게 한다.
 */
import { execFileSync } from 'node:child_process';
import path from 'node:path';

const FIXTURE_DIRS = ['lib', 'features', 'providers', 'app'] as const;
const fileFor = (layer: string) => path.join('src', layer, '__eslint_fixture__', 'probe.ts');
const ESLINT_BIN = path.join(path.dirname(require.resolve('eslint/package.json')), 'bin/eslint.js');

const SOURCE = `import { AppProviders } from '@/providers/app-providers';
import { Button } from '@/components/ui/button';
import type { HomeScreen } from '@/features/home/home-screen';
export const probe = [AppProviders, Button];
export type Probe = typeof HomeScreen;
`;

const COLOR_SOURCE = `import { Uniwind, useCSSVariable } from 'uniwind';
export const probe = [useCSSVariable, Uniwind.getCSSVariable('--color-red'), '#de141f'];
`;

type Message = { ruleId: string | null; severity: number; message: string };
const results: Record<string, Message[]> = {};

function lint(filename: string, input: string) {
  let raw = '';
  try {
    raw = execFileSync(
      process.execPath,
      [ESLINT_BIN, '--stdin', '--stdin-filename', filename, '--format', 'json'],
      { encoding: 'utf8', input }
    );
  } catch (error) {
    // eslint 는 위반이 있으면 exit 1 이다 — stdout 이 결과다.
    raw = (error as { stdout?: string }).stdout ?? '';
  }
  return (JSON.parse(raw) as { messages: Message[] }[]).flatMap(r => r.messages);
}

beforeAll(() => {
  for (const layer of FIXTURE_DIRS) results[layer] = lint(fileFor(layer), SOURCE);
  results.color = lint(fileFor('features'), COLOR_SOURCE);
  results.colorsFile = lint(path.join('src', 'lib', 'theme', 'use-colors.ts'), COLOR_SOURCE);
});

const providersError = (layer: string) =>
  (results[layer] ?? []).filter(
    m => m.severity === 2 && m.message.includes('providers는 루트 조립')
  );

const messagesFor = (layer: string) => JSON.stringify(results[layer] ?? []);

describe('import 경계', () => {
  it('providers 역류는 features 에서도 error 다 — 가장 중요한 곳이 조용히 비어 있었다', () => {
    expect(providersError('features')).toHaveLength(1);
  });

  it('shared 계층에서도 error 다', () => {
    expect(providersError('lib')).toHaveLength(1);
  });

  it('app 과 providers 는 providers 를 쓸 수 있다', () => {
    expect(providersError('app')).toHaveLength(0);
    expect(providersError('providers')).toHaveLength(0);
  });

  it('barrel 규칙이 좁은 블록에서 사라지지 않는다', () => {
    for (const layer of ['features', 'lib', 'app']) {
      expect(messagesFor(layer)).toContain('barrel 경유 권장');
    }
  });

  it('features 는 cross-feature, shared 는 계층 메시지를 받는다', () => {
    expect(messagesFor('features')).toContain('cross-feature 결합');
    expect(messagesFor('lib')).toContain('shared 계층은 features를 모릅니다');
  });
});

describe('색 가드레일', () => {
  const colorErrors = (key: string) =>
    (results[key] ?? []).filter(m => m.ruleId === 'no-restricted-syntax' && m.severity === 2);

  it('hex · useCSSVariable · getCSSVariable 은 error 다', () => {
    const messages = JSON.stringify(colorErrors('color'));
    expect(colorErrors('color')).toHaveLength(3);
    expect(messages).toContain('hex 색을 직접');
    expect(messages).toContain('useColors()(@/lib/theme/use-colors) 로 읽습니다');
    expect(messages).toContain('테마 전환을 따라가지 않습니다');
  });

  it('use-colors.ts 는 useCSSVariable 을 쓸 수 있지만 hex 와 getCSSVariable 은 못 쓴다', () => {
    const messages = JSON.stringify(colorErrors('colorsFile'));
    expect(messages).not.toContain('로 읽습니다');
    expect(messages).toContain('hex 색을 직접');
    expect(messages).toContain('테마 전환을 따라가지 않습니다');
  });
});
