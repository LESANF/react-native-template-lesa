...
- **`master` 에 직접 푸시하지 않는다.** 버전 브랜치(`0.0.1`)에 PR 을 올리고 **merge commit**
  으로 머지한다(`gh pr merge --merge`). squash 는 커밋 단위 이력과 `Co-Authored-By`
  트레일러를 하나로 뭉갠다. 브랜치는 태그(`v0.0.1`)와 이름이 겹치지 않게 `v` 를 뗀다.
