...
- **릴리즈는 모아서 낸다.** 오타·주석·코드 정리로는 버전을 올리지 않는다 — 태그 없이
  `master` 까지만 보내고 받는 쪽이 달라지는 변경(SDK 대응·버그·계약 변경)이 생겼을 때 같이 낸다.
  릴리즈 한 번에 사용자가 `npm publish` 와 OTP 를 친다.
...
- **`master` 에 직접 푸시하지 않고, PR 은 `master` 로만 연다.** `feature/xxx` → 로컬 머지
  (`git merge --no-ff`, PR 없음) → 버전 브랜치(`0.0.2`) → PR → `master`. 버전 브랜치로 가는
  PR 은 만들지 않는다 — GitHub 은 커밋을 처음 실어 온 PR 에 묶어서, 그 뒤의 `master` PR 에는
  머지 커밋만 남는다(CI 가 막는다). 작업 단위가 끝날 때마다 `master` PR 을 연다.
  **릴리즈는 master 머지가 아니라 태그다** — CLI 가 태그로 받으므로 master 가 앞서 있어도
  사용자에게 가지 않는다. 태그는 master 에서 단다.
...
| 푸시 이미지가 안 붙는다(제목·본문만)           | 이미지 URL 이 SVG 를 준다. iOS 첨부(NSE·notifee 둘 다 `UNNotificationAttachment`)는 JPEG·PNG·GIF 만. `placehold.co` 는 확장자 없으면 SVG — `.png` 를 붙인다 |
