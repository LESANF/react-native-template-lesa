/** 채널 식별자 · 서버 페이로드 계약. 흩어지면 첫 푸시가 엉뚱한 채널로 간다. */

/** `firebase.json` 의 messaging_android_notification_channel_id 와 **같아야** 한다. */
export const PUSH_CHANNEL_ID = 'high-priority';

/** Android 설정 화면에 노출. TODO(앱): 사용자용 문구로. */
export const PUSH_CHANNEL_NAME = 'High Priority Notifications';

/** 포그라운드 배너. bg·종료는 OS 가 그린다. KR 은 끈다. */
export const SHOW_FOREGROUND_NOTIFICATION = true;

/** 앞에서부터 첫 non-empty 가 이긴다. TODO(앱): 서버 계약대로 줄인다. */
